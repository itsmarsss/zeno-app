...
pub fn run_session_history_blocking(
    ...
    if let Some(value) = max_days {
        cmd.arg("--max-days").arg(value.max(1).to_string());
    }
...
pub fn run_exercise_history_blocking(limit: Option<u32>) -> Result<Value, String> {
    let root = project_root();
    let python_bin = resolve_python_bin(&root);
    let backend_dir = resolve_backend_dir(&root)?;
    ...
    cmd.current_dir(&backend_dir)
        .arg("-m")
        .arg("zeno_backend.data.exercise_history");
    if let Some(value) = limit {
        cmd.arg("--limit").arg(value.max(1).to_string());
    }
    ...
    let output = cmd
        .output()
        .map_err(|e| format!("Failed to read exercise history: {e}"))?;
    debug_log_python_output("exercise_history", &output);
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        let stdout = String::from_utf8_lossy(&output.stdout).to_string();
        return Err(format!(
            "Exercise history failed (code: {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
            stdout,
            stderr
        ));
...
pub fn run_activity_stats_blocking() -> Result<Value, String> {
    let root = project_root();
    let python_bin = resolve_python_bin(&root);
    let backend_dir = resolve_backend_dir(&root)?;
    ...
    let output = Command::new(python_bin)
        .current_dir(&backend_dir)
        .arg("-m")
        .arg("zeno_backend.data.activity_stats")
        .output()
        .map_err(|e| format!("Failed to read activity stats: {e}"))?;
    debug_log_python_output("activity_stats", &output);
    if !output.status.success() {
        let stderr = String::from_utf8_lossy(&output.stderr).to_string();
        let stdout = String::from_utf8_lossy(&output.stdout).to_string();
        return Err(format!(
            "Activity stats failed (code: {:?})\nstdout:\n{}\nstderr:\n{}",
            output.status.code(),
            stdout,
            stderr
        ));
